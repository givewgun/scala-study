...
object Monads extends App {
  ...
  val attempt = Attempt {
    throw new RuntimeException("My own monads")
  }
  ...
  val lazyInstance = Lazy {
    println("Instantiate")
    42
  }//no print cuz lazy and by-name

  val flatMappedInstance = lazyInstance.flatMap(x => Lazy(10 * x))
  val flatMappedInstance2 = lazyInstance.flatMap(x => Lazy(20 * x))
